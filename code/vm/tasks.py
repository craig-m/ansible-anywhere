...
def play_admin(c):
    """ playbook that configures centos8-admin. """
    playbook = "centos8-admin-playbook.yml"
    print("checking " + playbook)
    with c.cd(ansiblebase):
        c.run('ansible-lint ' + playbook + ' -v', pty=True)
    print("Using ansible_runner py int with " + playbook + " on localhost")
    import ansible_runner
    r = ansible_runner.run(
        artifact_dir = '/tmp/arts', 
        private_data_dir = '/tmp', 
        inventory = ansiblebase + '/localhost.ini', 
        playbook = ansiblebase + playbook,
        quiet = 'true')
    print("\nFinal status:")

...
# files created
c_testfile /etc/sysctl.d/no-ipv6.conf
c_testfile /etc/centos8vm/build_time.txt
c_testfile /etc/modprobe.d/vm-blacklist.conf
...
# network
grep -i "# Generated by NetworkManager" /etc/resolv.conf

# no Failed services
systemctl list-units | grep -i failed | wc -l | grep -q 0 || { logit 'failed system services'; exit 1; }

# check kernel options
grep -q 2 /proc/sys/kernel/randomize_va_space || { logit 'alsr is turned off'; exit 1; }
...
then
    # check UEFI enabled
    grep -q "64" /sys/firmware/efi/fw_platform_size || { logit 'no x86_64 EFI'; exit 1; }
    grep -q "/boot/efi vfat" /proc/mounts || { logit 'no x86_64 EFI'; exit 1; }

    # secure boot enabled
    #bootctl | head -n3 | grep -q "Secure Boot: enabled" || { logit 'secure boot not enabled'; exit 1; }
fi
